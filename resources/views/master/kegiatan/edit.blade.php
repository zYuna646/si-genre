...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-elephant-900">Edit Kegiatan: {{ $kegiatan->name }}</h1>
        <p class="text-gray-600">Perbarui informasi kegiatan PIKR</p>
    </div>

    <x-form-section title="Data Kegiatan" submit="{{ route('master.kegiatan.update', $kegiatan->id) }}">
        @method('PUT')
        <input type="hidden" name="pikr_id" value="{{ $kegiatan->pikr_id }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                >{{ old('tujuan', $kegiatan->tujuan) }}</textarea>
                @error('tujuan')
                    <p class="mt-1 text-sm text-old-brick-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <x-slot name="actions">
            <a href="{{ route('master.kegiatan.index', ['pikr_id' => $kegiatan->pikr_id]) }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50">
                Batal
            </a>
