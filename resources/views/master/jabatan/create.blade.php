...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-elephant-900">Tambah Jabatan untuk PIKR: {{ $pikr->nama }}</h1>
        <p class="text-gray-600">Tambahkan jabatan baru ke PIKR ini</p>
    </div>

    <!-- Select2 CSS -->
    <link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    
    <x-form-section title="Data Jabatan" submit="{{ route('master.jabatan.store', ['pikr_id' => $pikr->id]) }}">
        @csrf
            ...
            <div>
                ...
                >
                    @foreach($anggotas as $anggota)
                        <option 
                            value="{{ $anggota->id }}" 
                            {{ (old('anggota_ids') && in_array($anggota->id, old('anggota_ids'))) ? 'selected' : '' }}
                        >
                            {{ $anggota->nama }}
                        </option>
                    @endforeach
        ...
        <x-slot name="actions">
            <a href="{{ route('master.jabatan.index', ['pikr_id' => $pikr->id]) }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50">
                Batal
            </a>
